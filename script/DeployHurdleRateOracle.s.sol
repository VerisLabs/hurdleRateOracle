...
contract DeployHurdleRateOracle is Script {
    ...
    string constant DEFAULT_SOURCE = "const apiResponse = await Functions.makeHttpRequest({"
        "url: `https://api.maxapy.io/hurdle-rate`," "headers: {'Content-Type': 'application/json'}" "});"
        "if (apiResponse.error) throw Error('API Request Error');" "const { data } = apiResponse;"
    ...
    function run() external returns (HurdleRateOracle) {
        ...
        HurdleRateOracle oracle = new HurdleRateOracle(ROUTER, subscriptionId, DEFAULT_SOURCE);
